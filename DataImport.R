DataImport <- function(working.dir) {
    # Load packages
    library(data.table)
    library(tidyverse)
    library(quanteda)
    ...
    p <- 0.25    # Proportion of text to sample
    ...
    n.lines <- data.frame(dataset = c("blogs", "news", "twitter"), n.lines = c(blogs.n, news.n, twitter.n), proportion.sampled = c(p, p, p))
    return(n.lines)
}